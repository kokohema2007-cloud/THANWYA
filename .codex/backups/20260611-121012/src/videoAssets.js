import { getAuthToken } from './serverApi.js';
...
export async function loadVideoAsset(id) {
  ...
  try {
    const token = getAuthToken();
    const response = await fetch(`/api/uploads/video/${encodeURIComponent(id)}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    });
    if (response.ok) {
...
export async function deleteVideoAsset(id) {
  try {
    const token = getAuthToken();
    await fetch(`/api/uploads/video/${encodeURIComponent(id)}`, {
      method: 'DELETE',
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    });
    return;
