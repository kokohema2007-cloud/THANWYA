...
function App() {
  ...
  return (
    <div className="app">
      ...
      {!session ? (
        publicScreen === 'student' ? <StudentPortalScreen onLogin={loginWithCode} /> : <AuthScreen onLogin={loginWithCode} onOpenStudentPortal={() => setPublicScreen('student')} />
      ) : session.role === 'admin' ? (
        <AdminDashboard
          codes={codes}
          setCodes={setCodes}
...
function AuthScreen({ onLogin, onOpenStudentPortal }) {
...
function LibraryPanelTree({ library, setLibrary, lessonCodes, setLessonCodes }) {
  ...
  function removeTeacher(teacherTargetId) {
    ...
    if (teacher) {
      collectVideoAssetIdsFromFolders(teacher.lessons ?? []).forEach((assetId) => deleteVideoAsset(assetId).catch(() => {}));
    }
    updateSubject(selectedSubject.id, (subject) => ({
      ...subject,
      teachers: subject.teachers.filter((teacher) => teacher.id !== teacherTargetId),
  ...
  function removeFolder(folderId) {
    ...
    if (folder) {
      collectVideoAssetIdsFromFolders([folder]).forEach((assetId) => deleteVideoAsset(assetId).catch(() => {}));
    }
  ...
  function removeResource(type, resourceId) {
    ...
    if (type === 'videos') {
      ...
      if (video?.assetId) deleteVideoAsset(video.assetId).catch(() => {});
    }
    commitTree((tree) =>
      updateFolderById(tree, selectedFolder.id, (folder) => ({
        ...folder,
...
function StudentDashboardTree({ profile, library, setLibrary, accessRecords = [], studentToken, homeSignal, onMainScreenChange }) {
  ...
  useEffect(() => {
    ...
    resolveSource().catch(() => setResolvedVideoSource(selectedVideo?.source ?? ''));
